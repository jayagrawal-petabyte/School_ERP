...
const hasDuplicateExam = async ({ name, term, academic_year, class_id }, excludeId = null) => {
...
const updateExam = async (id, payload) => {
  ...
  if (await hasDuplicateExam(merged, id)) {
    throw new AppError('Exam already exists for the same class, term, and academic year', 409);
  }

  const exam = await examRepository.updateExam(id, updates);
