...
import { TeacherProfileView } from '../types';

type TeacherProfileRouteProp = RouteProp<RootStackParamList, 'TeacherProfile'>;
type TeacherProfileNavigationProp = NativeStackNavigationProp<RootStackParamList, 'TeacherProfile'>;

interface Props {
  route: TeacherProfileRouteProp;
...
export default function TeacherProfileScreen({ route, navigation }: Props) {
  ...
  useEffect(() => {
    setLoading(true);
    ProfileService.getTeacherProfile(userId).then((result) => {
      setProfile(result);
      setLoading(false);
    });
  }, [userId]);
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
            onPress={() => navigation.navigate('EditProfile', { userId: profile.id, role: 'teacher', currentName: profile.full_name })}
          >
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: FONT_SIZE.xs, color: COLORS.textMuted, marginBottom: SPACING.xs },
