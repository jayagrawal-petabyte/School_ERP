...
type AssignmentListRouteProp = RouteProp<RootStackParamList, "AssignmentList">;

type AssignmentListNavigationProp = NativeStackNavigationProp<
    RootStackParamList,
    "AssignmentList"
>;

interface Props {
    route: AssignmentListRouteProp;
    navigation: AssignmentListNavigationProp;
}
...
const mapAssignment = (
    assignment: any,
    status: "pending" | "submitted" | "late" | "graded" = "pending",
): Assignment => {
    return {
        ...
        maxMarks: assignment.maxMarks || assignment.max_marks || 0,

        assignedBy: assignment.createdBy || assignment.created_by || "Teacher",
...
export default function AssignmentListScreen({ route, navigation }: Props) {
    ...
    const loadData = async (showLoader = true) => {
        ...
        try {
            ...
            const assignmentList = Array.isArray(response)
                ? response
                : response?.assignments || [];

            const classAssignments = assignmentList.filter(
                (item: any) =>
                    !classId ||
                    String(item.classId || item.class_id) === String(classId),
            );

            const formattedAssignments = await Promise.all(
                classAssignments.map(async (item: any) => {
                    try {
                        const submission =
                            await assignmentApi.getSubmissionStatus(item.id);
    ...
    return (
        <SafeAreaView style={styles.safeContainer} edges={["top"]}>
            <AppHeader title="Assignments" showBackButton />
            ...
            <View style={styles.container}>
                ...
                <View style={styles.filterTabsRow}>
                    {["all", "pending", "submitted", "late", "graded"].map(
                        (filter) => (
                            <TouchableOpacity
                                key={filter}
                                style={[
